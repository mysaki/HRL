...
# from Nets.Star_net_rnn import STAR as STAR_ONLY
from Nets.Star_net_KURL import STAR
from get_test_track_policy import *
from Nets.KURL_Net import Track_Net
...
def test_ppo(args: argparse.Namespace = get_args()) -> None:
    ...
    # log
    log_path = os.path.join(args.logdir, "track_{}_KURL_join_ppo_{}_{}_{}_{}".format(
    ...
    APNet = Track_Net(
        input_dim=np.prod(args.state_shape), device=args.device, feature_dim=512)
    ...
    inside_model = get_Both_STAR_only_policy(path = "./Log/join_train_track_ppo_1_18_10_29")
    inside_model.eval()
    policy = PPOPolicy(
        actor=actor,
        critic=critic,
