...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument("--task-1", type=str, default="Navigation-v0")
...
def test_ppo(args: argparse.Namespace = get_args()) -> None:
    ...
    # log
    log_path = os.path.join(args.logdir, "track_nav_{}_ppo_{}_{}_{}_{}".format(
        label,time.month, time.day, time.hour, time.minute, time.second))
    log_model_name = "nav_train_ppo_12_29_11_27"
    ...
    if args.test == False:  # trainer
        ...
        env_1 = gym.make(args.task_1, headless=args.headless,mode=args.test)
        envs = []
        for i in range(0,args.training_num,2):
            envs.append(lambda i=i: gym.make(args.task, headless=args.headless,mode=args.test))
            envs.append(lambda i=i+1: gym.make(args.task_1, headless=args.headless,mode=args.test))
        # track_env = gym.make("Navigation-v0", headless=args.headless,mode=args.test)
        train_envs = ShmemVectorEnv(envs)
        envs = []
        for i in range(0,args.test_num,2):
            envs.append(lambda i=i: gym.make(args.task, headless=args.headless,mode=args.test))
            envs.append(lambda i=i+1: gym.make(args.task_1, headless=args.headless,mode=args.test))        
        test_envs = ShmemVectorEnv(envs)
        # seed
