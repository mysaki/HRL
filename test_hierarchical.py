...
policy_set = [policy_NAV,policy_KURL,policy_hierarchical,policy_rule_based]#, policy_hierarchical_early]
for policy in policy_set:
    policy.eval()
model_name = ["Nav","KURL","Hierarchical","Rule_Based"]#,"Hierarchical_DQN_EARLY"]
...
for episode in range(test_episodes):
    ...
    for i in range(policy_num):
        ...
        while not done:
            episode_count += 1
            batch = Batch()
            if model_name[i] == "KURL":
                state = np.concatenate([state[:2],np.array([1,0]),state[2:]])
