...
policy_ACSTAR = get_Both_STAR_policy(path="./Log/join_train_track_ppo_4_18_18_14")
policy_ACSTAR_rnn = get_Both_STAR_rnn_policy(path="./Log/join_train_track_ppo_4_20_9_51")
policy_ACSTAR_mha = get_Both_STAR_mha_policy(path="./Log/join_train_track_ppo_4_18_18_0")
policy_ACSTAR_only = get_Both_STAR_only_policy(
    path="./Log/join_train_track_ppo_4_20_9_55"
)
policy_set = [policy_ACSTAR,policy_ACSTAR_rnn,policy_ACSTAR_mha,policy_ACSTAR_only]
...
model_name = ["RNN_MHA","RNN",'MHA','MLP_ONLY']
# model_name = ['Bacteria_APF']
task = "Navigation-v0"
test_count = 0
episode_count = 0
...
for episode in range(test_episodes):
    ...
    for i in range(policy_num):
        ...
        epi_avg_dist = 0
        epi_collisions = 0
        if model_name[i] == "Bacteria_APF":
            obs = []
            ori_obs = list(str(obstacles[3]).split(" "))
            for x in range(0,len(ori_obs),3):
                obstacle = [float(ori_obs[x]),float(ori_obs[x+1]),float(ori_obs[x+2])]
                obs.append(obstacle)
            policy = Bacteria_policy(obs,init_tracker_pos[:2],init_target_pos[:2])
        else:
            policy = policy_set[i]
        ...
        while not done:
            ...
            batch.info = {}
            if model_name[i] == "Bacteria_APF":
                new_tracker_coordinates = policy.step(env.tracker.get_2d_pose()[:2])
                batch.info['bacteria'] = 1
                batch.info['new_tracker_coordinates'] = new_tracker_coordinates
