...
class Track_Net(nn.Module):
    ...
    def __init__(
        ...
        self.fc1 = MLP(input_dim=feature_dim,output_dim=1024,hidden_sizes=[256,256])
    ...
    def forward(
        ...
        r"""Mapping: s -> Q(s, \*)."""
        if len(obs.shape) <2:
            if isinstance(obs, torch.Tensor) and obs.device.type == 'cuda':
                obs = np.array([obs.detach().cpu().numpy()])
            else:
                obs = np.array([obs])
