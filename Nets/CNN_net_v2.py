...
class CNN_Net(nn.Module):
    ...
    def __init__(
        ...
        self.net = nn.Sequential(
            layer_init(nn.Conv2d(c, 512, kernel_size=8, stride=4)),
            nn.ReLU(inplace=True),
            layer_init(nn.Conv2d(512, 256, kernel_size=4, stride=2)),
            nn.ReLU(inplace=True),
            layer_init(nn.Conv2d(256, 128, kernel_size=3, stride=1)),
            nn.ReLU(inplace=True), nn.Flatten()
        )
        with torch.no_grad():
            self.output_dim = np.prod(self.net(torch.zeros(1, c, h, w)).shape[1:])
        self.net = nn.Sequential(
                self.net, layer_init(nn.Linear(self.output_dim, 128)),
                nn.ReLU(inplace=True)
            )
