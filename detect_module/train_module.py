...
# 设置路径
data_dir = "/media/hp/新加卷/XNW/Hiearchical_RL/detect_module/labeled_images/real"
img_height, img_width = 250, 250  # 图像大小
time = datetime.datetime.now()  # 获取当前时间
log_dir = "/media/hp/新加卷/XNW/Hiearchical_RL/detect_module/detection_record"
...
epochs = 1000
auc_file_path = os.path.join(log_path, 'training_aucs.csv')
with open(auc_file_path, mode='w', newline='') as csv_file:
    fieldnames = ['Epoch', 'fpr','tpr','auc']
...
# 打开 CSV 文件以写入损失数据
csv_file_path = os.path.join(log_path, 'training_losses.csv')
with open(csv_file_path, mode='w', newline='') as csv_file:
    ...
    for epoch in range(epochs):
        ...
        # 将损失数据写入 CSV 文件
        writer.writerow({'Epoch': epoch, 'Loss': average_loss})
        ...
        if (epoch + 1) % 10 == 0:
            ...
            with open(auc_file_path, mode='w', newline='') as csv_file:
                fieldnames = ['Epoch', 'fpr','tpr','auc']
                auc_writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
                auc_writer.writerow({'Epoch':epoch,'fpr':fpr,'tpr':tpr,'auc':roc_auc})
            print(f"Cross-validation ROC AUC after epoch {epoch + 1}: {roc_auc:.2f}")
            # 绘制并保存ROC曲线
            plt.figure()
            plt.plot(fpr, tpr, color="blue", lw=2, label=f"ROC 曲线 (面积= {roc_auc:.2f})")
            plt.plot([0, 1], [0, 1], color="red", lw=2, linestyle="--")
            plt.xlim([0.0, 1.0])
            plt.ylim([0.0, 1.05])
            plt.xlabel("假阳性率")
            plt.ylabel("真阳性率")
            plt.title("Receiver Operating Characteristic曲线")
            plt.legend(loc="lower right")

            # 保存ROC曲线图像
            plt.savefig(f"{log_path}/roc_curve_epoch_{epoch + 1}.jpg",dpi=300)  # 保存文件名
            plt.close()  # 关闭当前图形以释放内存

# 绘制损失曲线
plt.figure()
plt.plot(range(1, 1 + epochs), losses, marker=".", linestyle="-", color="blue")
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.title("损失曲线")
