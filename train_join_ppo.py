...
def test_ppo(args: argparse.Namespace = get_args()) -> None:
    ...
    CPNet = STAR(input_dim=np.prod(args.state_shape), feature_dim=256, device=args.device,hidden_dim=[128,128])
    ...
    APNet = STAR(input_dim=np.prod(args.state_shape), feature_dim=256, device=args.device,hidden_dim=[128,128])
